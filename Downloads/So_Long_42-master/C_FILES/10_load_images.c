...
t_ent	**get_images_from_file(t_mlx_data *mlx_data, t_ent_type type, \
	char *path, float scale_factor, int amount)
{
	t_vec2	end_scale;
	t_ent	**e;
	char	*nbr;
	...
	while (++i < amount)
	{
		...
		free(nbr);
		e[i]->cur_frame = add_img(full_path, &e[i]->size.x, &e[i]->size.y, \
			mlx_data->mlx);
		if (scale_factor != 1)
		{
			...
			if (end_scale.x > 1 && end_scale.y > 1)
				e[i]->cur_frame = rescale_image(mlx_data, e[i]->cur_frame, &e[i]->size, end_scale);
		}
		e[i]->type = type;
...
void	load_images(t_mlx_data *mlx_data)
{
	...
	if (!mlx_data->alll_images)
		return;
	mlx_data->alll_images[0] = get_images_from_file(mlx_data, \
		trees, TREES_SPRITES_PATH, .4, 7);
	mlx_data->alll_images[1] = get_images_from_file(mlx_data, \
		deco, BUSH_SPRITES_PATH, .4, 10);

...
int save_to_file(t_mlx_data *mlx_data, int len)
{
	...
	if (mlx_data->background)
		...
		mlx_data->background->size.x, mlx_data->background->size.y, \
		background, mlx_data->background->frame_path);
	i = -1;
	while (++i < len)
	{
		...
		if (ent)
		{
			...
			ent->size.x, ent->size.y, ent->type, ent->frame_path);
			printf("SVD: PATH %s | TYPE %d | POS[%d,%d] SIZE[%d,%d]\n", \
			ent->frame_path, ent->type, ent->pos.x, ent->pos.y, ent->size.x, \
			ent->size.y);
		}
	}
	fclose(file);
...
char	*find_next_occurrence(const char *str, int start_index)
{
	...
	len = 0;
	while (str[start_index + len] && str[start_index + len] >= '0' && \
	str[start_index + len] <= '9')
		len++;
...
int load_from_file(t_mlx_data *mlx_data, char *file_path)
{
	...
	while (line != NULL)
	{
		...
		if (e != NULL)
		{
			...
			e->cur_frame = add_img(e->frame_path, &img_size.x, &img_size.y, mlx_data);
			if (e->size.x != img_size.x || e->size.y != img_size.y)
				e->cur_frame = rescale_image(mlx_data, e->cur_frame, &img_size, e->size);
			if (line[0] == 'b')
